---
name: red-team
description: Falsify "it's done / it works / all good" claims. Use after any "done," "passing," "merged," "all good" claim, after /ship-check's Stage 1 verify, or any time the cost of being wrong is high. This is the single source of truth for the adversarial falsification pass — /red-team and /ship-check Stage 2 both invoke it verbatim, never duplicate it. Includes the AI-native-coverage and voice-coverage attack lanes (can an agent/API/CLI/MCP reach every new capability; can the voice command path reach every new screen and capability).
---

# Red-Team — Falsify "It's Done / All Good"

**Announce at start:** "I'm using the red-team skill to attack the claims from this session before they ship."
...
This is **falsification, not a checklist**. `/ship-check` Stage 1 walks a broad readiness audit. This skill does one thing: takes claims already believed true and **attacks them with disconfirming evidence**. Run it standalone (`/red-team`) after a "done", or as `/ship-check` Stage 2, immediately after Stage 1 Verify.
...
- **(a) AI-native coverage** — for every new capability shipped this session, can it be reached without a human finger on glass? Enumerate the new capability's entry points and check each: is there an agent tool / MCP tool, a CLI command, or an API route that reaches it, or is it **tap-only** (UI-gesture-only, no programmatic path)? A capability an agent cannot discover or invoke is a coverage gap even if the UI works perfectly — write it as a finding with a one-line fix (e.g. "add `create_reminder` to the MCP tool catalogue" / "expose `POST /api/reminders`" / "add a `reminders create` CLI verb").
- **(b) Voice coverage** — for any repo with a voice/Stream surface: for every new screen and capability shipped this session, can the voice command path reach it? Check the navigate allowlist (is the new screen/route in it?), the tool catalogue (is the new action registered?), and the command router (does an utterance actually map to it, not just "the intent would theoretically parse")? A new screen unreachable by voice, or a new action missing from the tool catalogue, is a coverage gap — write it as a finding with a one-line fix (e.g. "add `/coach/streaks` to the navigate allowlist" / "register `log_workout` in the voice tool catalogue"). Skip this lane only when the repo genuinely has no voice surface — say so explicitly rather than silently omitting it.
...
| <e.g. "new capability is agent-reachable"> | Grep MCP/CLI/API catalogues for the new capability | found in tool catalogue, invoked once | ✅ SURVIVED |
| <e.g. "new screen is voice-reachable"> | Grep navigate allowlist + voice tool catalogue + command router | missing from navigate allowlist | 🔴 CRACKED |
| ... | ... | ... | ... |
...
- AI-native coverage: <every new capability's agent/API/CLI/MCP reachability, gap or none>
- Voice coverage: <every new screen/capability's navigate-allowlist/tool-catalogue/router reachability, gap or none, or "n/a — no voice surface">
...
If any claim ends the pass as `🔴 CRACKED` with no fix landed (a genuine external blocker), the verdict is **NOT SURVIVED** — say so plainly, first line of the report. A caller composing this skill (e.g. `/ship-check` Stage 2) MUST treat anything other than **SURVIVED** as a stage failure and stop before the next stage.

---

## Integration with other skills

- **`/ship-check` Stage 2** invokes this skill verbatim, immediately after Stage 1 Verify and before Stage 3 Definition of Done. Ship-check does not restate this skill's phases — it loads this file and gates Stage 3/4 on the **Verdict** line.
- **`definition-of-done`** skill's Verification section (row 7.2) expects a red-team pass to have run; this skill is what satisfies that row.
- **`superpowers:requesting-code-review` / `receiving-code-review`** — the review fan-out those skills wrap is a valid disconfirming experiment source for code-correctness claims (see Phase 1 "Compose, don't reinvent").
...
- **AI-native and voice coverage are mandatory lanes, not optional extras.** Every red-team pass runs Phase 2(a) unconditionally, and Phase 2(b) unless the repo genuinely has no voice surface (say so explicitly if skipped).
